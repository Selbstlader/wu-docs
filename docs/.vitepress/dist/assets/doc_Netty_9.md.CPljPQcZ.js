...
<span class="line"><span>        });</span></span></code></pre></div><p>这样，在客户端侧，Netty 中 IO 事件相关的回调就能够回调到我们的 <code>ClientHandler</code>。</p><p>同样，我们给服务端引导类 <code>ServerBootstrap</code> 也配置一个逻辑处理器 <code>ServerHandler</code></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ServerHandler extends ChannelInboundHandlerAdapter {</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>这里，我们按照前面所描述的三个步骤来分别实现，在编码的环节，我们把 <code>PacketCodeC</code> 变成单例模式，然后把 <code>ByteBuf</code> 分配器抽取出一个参数，这里第一个实参 <code>ctx.alloc()</code> 获取的就是与当前连接相关的 <code>ByteBuf</code> 分配器，建议这样来使用。</p><p>写数据的时候，我们通过 <code>ctx.channel()</code> 获取到当前连接（Netty 对连接的抽象为 Channel，后面小节会分析），然后调用 <code>writeAndFlush()</code> 就能把二进制数据写到服务端。这样，客户端发送登录请求的逻辑就完成了，接下来，我们来看一下，服务端接受到这个数据之后是如何来处理的。</p><h3 id="服务端处理登录请求" tabindex="-1">服务端处理登录请求 <a class="header-anchor" href="#服务端处理登录请求" aria-label="Permalink to &quot;服务端处理登录请求&quot;">​</a></h3><blockquote><p>ServerHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public void channelRead(ChannelHandlerContext ctx, Object msg) {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>我们向服务端引导类 <code>ServerBootstrap</code> 中添加了逻辑处理器 <code>ServerHandler</code> 之后，Netty 在收到数据之后，会回调 <code>channelRead()</code> 方法，这里的第二个参数 <code>msg</code>，在我们这个场景中，可以直接强转为 <code>ByteBuf</code>，为什么 Netty 不直接把这个参数类型定义为 <code>ByteBuf</code> ？我们在后续的小节会分析到。</p><p>拿到 <code>ByteBuf</code> 之后，首先要做的事情就是解码，解码出 java 数据包对象，然后判断如果是登录请求数据包 <code>LoginRequestPacket</code>，就进行登录逻辑的处理，这里，我们假设所有的登录都是成功的，<code>valid()</code> 方法返回 true。 服务端校验通过之后，接下来就需要向客户端发送登录响应，我们继续编写服务端的逻辑。</p><h2 id="服务端发送登录响应" tabindex="-1">服务端发送登录响应 <a class="header-anchor" href="#服务端发送登录响应" aria-label="Permalink to &quot;服务端发送登录响应&quot;">​</a></h2><h3 id="服务端处理登录响应" tabindex="-1">服务端处理登录响应 <a class="header-anchor" href="#服务端处理登录响应" aria-label="Permalink to &quot;服务端处理登录响应&quot;">​</a></h3><blockquote><p>ServerHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>LoginResponsePacket loginResponsePacket = new LoginResponsePacket();</span></span>
