import{_ as n,c as a,o as s,a4 as e}from"./chunks/framework.DLF8A2I8.js";const g=JSON.parse('{"title":"客户端启动流程","description":"","frontmatter":{},"headers":[],"relativePath":"doc/Netty/5.md","filePath":"doc/Netty/5.md"}'),p={name:"doc/Netty/5.md"},t=e(`<h2 id="本资源由-itjc8-com-收集整理" tabindex="-1">本资源由 itjc8.com 收集整理 <a class="header-anchor" href="#本资源由-itjc8-com-收集整理" aria-label="Permalink to &quot;本资源由 itjc8.com 收集整理&quot;">​</a></h2><h1 id="客户端启动流程" tabindex="-1">客户端启动流程 <a class="header-anchor" href="#客户端启动流程" aria-label="Permalink to &quot;客户端启动流程&quot;">​</a></h1><p>上一小节，我们已经学习了 Netty 服务端启动的流程，这一小节，我们来学习一下 Netty 客户端的启动流程。</p><h2 id="客户端启动-demo" tabindex="-1">客户端启动 Demo <a class="header-anchor" href="#客户端启动-demo" aria-label="Permalink to &quot;客户端启动 Demo&quot;">​</a></h2><p>对于客户端的启动来说，和服务端的启动类似，依然需要线程模型、IO 模型，以及 IO 业务处理逻辑三大参数，下面，我们来看一下客户端启动的标准流程</p><blockquote><p>NettyClient.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public class NettyClient {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>});</span></span></code></pre></div><p>重新连接的时候，依然是调用一样的逻辑，因此，我们把建立连接的逻辑先抽取出来，然后在重连失败的时候，递归调用自身</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>private static void connect(Bootstrap bootstrap, String host, int port) {</span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>    });</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>上面这一段便是带有自动重连功能的逻辑，可以看到在连接建立失败的时候，会调用自身进行重连。</p><p>但是，通常情况下，连接建立失败不会立即重新连接，而是会通过一个指数退避的方式，比如每隔 1 秒、2 秒、4 秒、8 秒，以 2 的幂次来建立连接，然后到达一定次数之后就放弃连接，接下来我们就来实现一下这段逻辑，我们默认重试 5 次</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>connect(bootstrap, &quot;juejin.im&quot;, 80, MAX_RETRY);</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>    });</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>从上面的代码可以看到，通过判断连接是否成功以及剩余重试次数，分别执行不同的逻辑</p><ol><li>如果连接成功则打印连接成功的消息</li><li>如果连接失败但是重试次数已经用完，放弃连接</li><li>如果连接失败但是重试次数仍然没有用完，则计算下一次重连间隔 <code>delay</code>，然后定期重连</li></ol><p>在上面的代码中，我们看到，我们定时任务是调用 <code>bootstrap.config().group().schedule()</code>, 其中 <code>bootstrap.config()</code> 这个方法返回的是 <code>BootstrapConfig</code>，他是对 <code>Bootstrap</code> 配置参数的抽象，然后 <code>bootstrap.config().group()</code> 返回的就是我们在一开始的时候配置的线程模型 <code>workerGroup</code>，调 <code>workerGroup</code> 的 <code>schedule</code> 方法即可实现定时任务逻辑。</p><p>在 <code>schedule</code> 方法块里面，前面四个参数我们原封不动地传递，最后一个重试次数参数减掉一，就是下一次建立连接时候的上下文信息。读者可以自行修改代码，更改到一个连接不上的服务端 Host 或者 Port，查看控制台日志就可以看到5次重连日志。</p><p>以上就是实现指数退避的客户端重连逻辑，接下来，我们来一起学习一下，客户端启动，我们的引导类<code>Bootstrap</code>除了指定线程模型，IO 模型，连接读写处理逻辑之外，他还可以干哪些事情？</p><h2 id="客户端启动其他方法" tabindex="-1">客户端启动其他方法 <a class="header-anchor" href="#客户端启动其他方法" aria-label="Permalink to &quot;客户端启动其他方法&quot;">​</a></h2><h3 id="attr-方法" tabindex="-1">attr() 方法 <a class="header-anchor" href="#attr-方法" aria-label="Permalink to &quot;attr() 方法&quot;">​</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>bootstrap.attr(AttributeKey.newInstance(&quot;clientName&quot;), &quot;nettyClient&quot;)</span></span></code></pre></div><p><code>attr()</code> 方法可以给客户端 Channel，也就是<code>NioSocketChannel</code>绑定自定义属性，然后我们可以通过<code>channel.attr()</code>取出这个属性，比如，上面的代码我们指定我们客户端 Channel 的一个<code>clientName</code>属性，属性值为<code>nettyClient</code>，其实说白了就是给<code>NioSocketChannel</code>维护一个 map 而已，后续在这个 <code>NioSocketChannel</code> 通过参数传来传去的时候，就可以通过他来取出设置的属性，非常方便。</p><h3 id="option-方法" tabindex="-1">option() 方法 <a class="header-anchor" href="#option-方法" aria-label="Permalink to &quot;option() 方法&quot;">​</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>Bootstrap</span></span>
