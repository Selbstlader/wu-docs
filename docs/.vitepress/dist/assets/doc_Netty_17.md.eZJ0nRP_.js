...
<span class="line"><span>        }</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ol><li>我们在这个管理类中，把所有要管理的控制台指令都塞到一个 map 中。</li><li>执行具体操作的时候，我们先获取控制台第一个输入的指令，这里以字符串代替，比较清晰（这里我们已经实现了上小节课后思考题中的登出操作），然后通过这个指令拿到对应的控制台命令执行器执行。</li></ol><p>这里我们就拿创建群聊举个栗子：首先，我们在控制台输入 <code>createGroup</code>，然后我们按下回车，就会进入 <code>CreateGroupConsoleCommand</code> 这个类进行处理</p><blockquote><p>CreateGroupConsoleCommand.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public class CreateGroupConsoleCommand implements ConsoleCommand {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>抽取出控制台指令执行器之后，客户端控制台逻辑已经相对之前清晰很多了，可以非常方便地在控制台模拟各种在 IM 聊天窗口的操作，接下来，我们就来看一下如何创建群聊。</p><h2 id="_3-创建群聊的实现" tabindex="-1">3. 创建群聊的实现 <a class="header-anchor" href="#_3-创建群聊的实现" aria-label="Permalink to &quot;3\\. 创建群聊的实现&quot;">​</a></h2><h3 id="_3-1-客户端发送创建群聊请求" tabindex="-1">3.1 客户端发送创建群聊请求 <a class="header-anchor" href="#_3-1-客户端发送创建群聊请求" aria-label="Permalink to &quot;3.1 客户端发送创建群聊请求&quot;">​</a></h3><p>通过我们前面讲述控制台逻辑的重构，我们已经了解到我们是发送一个 <code>CreateGroupRequestPacket</code> 数据包到服务端，这个数据包的格式为：</p><blockquote><p>CreateGroupRequestPacket.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public class CreateGroupRequestPacket extends Packet {</span></span>
<span class="line"><span>    private List&lt;String&gt; userIdList;</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>它只包含了一个列表，这个列表就是需要拉取群聊的用户列表，接下来我们看下服务端如何处理的。</p><h3 id="_3-2-服务端处理创建群聊请求" tabindex="-1">3.2 服务端处理创建群聊请求 <a class="header-anchor" href="#_3-2-服务端处理创建群聊请求" aria-label="Permalink to &quot;3.2 服务端处理创建群聊请求&quot;">​</a></h3><p>我们依然是创建一个 handler 来处理新的指令。</p><blockquote><p>NettyServer.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>.childHandler(new ChannelInitializer&lt;NioSocketChannel&gt;() {</span></span>
...
<span class="line"><span>        // ...</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>});</span></span></code></pre></div><p>接下来，我们来看一下这个 handler 具体做哪些事情</p><blockquote><p>CreateGroupRequestHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public class CreateGroupRequestHandler extends SimpleChannelInboundHandler&lt;CreateGroupRequestPacket&gt; {</span></span>
<span class="line"><span>    @Override</span></span>
...
<span class="line"><span>        // ...</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>});</span></span></code></pre></div><p>然后，在我们的应用程序里面，我们仅仅是把创建群聊成功之后的具体信息打印出来。</p><blockquote><p>CreateGroupResponseHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public class CreateGroupResponseHandler extends SimpleChannelInboundHandler&lt;CreateGroupResponsePacket&gt; {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>    @Override</span></span>
