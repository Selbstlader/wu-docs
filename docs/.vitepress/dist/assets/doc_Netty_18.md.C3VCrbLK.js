...
<span class="line"><span>        // ...</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>接下来，就轮到服务端来处理加群请求了。</p><h3 id="_2-2-服务端处理群加入请求" tabindex="-1">2.2 服务端处理群加入请求 <a class="header-anchor" href="#_2-2-服务端处理群加入请求" aria-label="Permalink to &quot;2.2 服务端处理群加入请求&quot;">​</a></h3><p>服务端的 pipeline 中添加对应的 handler - <code>JoinGroupRequestHandler</code></p><blockquote><p>NettyServer.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span></span></span>
...
<span class="line"><span>        // ..</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>});</span></span></code></pre></div><p><code>JoinGroupRequestHandler</code> 的具体逻辑为</p><blockquote><p>JoinGroupRequestHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JoinGroupRequestHandler extends SimpleChannelInboundHandler&lt;JoinGroupRequestPacket&gt; {</span></span>
<span class="line"><span>    @Override</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ol><li>首先，通过 groupId 拿到对应的 <code>ChannelGroup</code>，之后，只需要调用 <code>ChannelGroup.add()</code> 方法，将加入群聊的用户的 channel 添加进去，服务端即完成了加入群聊的逻辑。</li><li>然后，构造一个加群响应，填入 groupId 之后，调用 <code>writeAndFlush()</code> 发送给加入群聊的客户端。</li></ol><h3 id="_2-3-客户端处理群加入响应" tabindex="-1">2.3 客户端处理群加入响应 <a class="header-anchor" href="#_2-3-客户端处理群加入响应" aria-label="Permalink to &quot;2.3 客户端处理群加入响应&quot;">​</a></h3><p>我们在客户端的 pipeline 中添加对应的 handler - <code>JoinGroupResponseHandler</code> 来处理加群之后的响应</p><blockquote><p>NettyClient.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>.handler(new ChannelInitializer&lt;SocketChannel&gt;() {</span></span>
<span class="line"><span>    @Override</span></span>
...
<span class="line"><span>        // ...</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>});</span></span></code></pre></div><p><code>JoinGroupResponseHandler</code> 对应的逻辑为</p><blockquote><p>JoinGroupResponseHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JoinGroupResponseHandler extends SimpleChannelInboundHandler&lt;JoinGroupResponsePacket&gt; {</span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>该处理器的逻辑很简单，只是简单的将加群的结果输出到控制台，实际生产环境 IM 可能比这个要复杂，但是修改起来也是非常容易的。至此，加群相关的逻辑到这里就结束了。</p><h2 id="_3-群的退出" tabindex="-1">3. 群的退出 <a class="header-anchor" href="#_3-群的退出" aria-label="Permalink to &quot;3\\. 群的退出&quot;">​</a></h2><p>关于群的退出和群的加入逻辑非常类似，这里展示一下关键代码，完整代码请参考 <a href="https://github.com/lightningMan/flash-netty" target="_blank" rel="noreferrer">github</a> <strong>对应本小节分支</strong></p><p>服务端退群的核心逻辑为 <code>QuitGroupRequestHandler</code></p><blockquote><p>QuitGroupRequestHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class QuitGroupRequestHandler extends SimpleChannelInboundHandler&lt;QuitGroupRequestPacket&gt; {</span></span>
<span class="line"><span>    @Override</span></span>
...
<span class="line"><span>        // ...</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>接着，轮到服务端来处理获取成员列表请求。</p><h3 id="_4-2-服务端处理获取成员列表请求" tabindex="-1">4.2 服务端处理获取成员列表请求 <a class="header-anchor" href="#_4-2-服务端处理获取成员列表请求" aria-label="Permalink to &quot;4.2 服务端处理获取成员列表请求&quot;">​</a></h3><p>服务端的 pipeline 中添加对应的 handler - <code>ListGroupMembersRequestHandler</code></p><blockquote><p>NettyServer.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span></span></span>
<span class="line"><span>.childHandler(new ChannelInitializer&lt;NioSocketChannel&gt;() {</span></span>
<span class="line"><span>    protected void initChannel(NioSocketChannel ch) {</span></span>
...
<span class="line"><span>        // ..</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>});</span></span></code></pre></div><p><code>ListGroupMembersRequestHandler</code> 的具体逻辑为</p><blockquote><p>ListGroupMembersRequestHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ListGroupMembersRequestHandler extends SimpleChannelInboundHandler&lt;ListGroupMembersRequestPacket&gt; {</span></span>
<span class="line"><span>    protected void channelRead0(ChannelHandlerContext ctx, JoinGroupRequestPacket requestPacket) {</span></span>
...
<span class="line"><span>        ctx.channel().writeAndFlush(responsePacket);</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ol><li>首先，我们通过 groupId 拿到对应的 <code>ChannelGroup</code>。</li><li>接着，我们创建一个 sessionList 用来装载群成员信息，我们遍历 channel 的每个 session，把对应的用户信息装到 sessionList 中，实际生产环境中，这里可能会构造另外一个对象来装载用户信息而非 Session，这里我们就简单粗暴点了，改造起来不难。</li><li>最后，我们构造一个获取成员列表的响应指令数据包，填入 groupId 和群成员的信息之后，调用 <code>writeAndFlush()</code> 发送给发起获取成员列表的客户端。</li></ol><p>最后，就剩下客户端来处理获取群成员列表的响应了。</p><h3 id="_4-3-客户端处理获取成员列表响应" tabindex="-1">4.3 客户端处理获取成员列表响应 <a class="header-anchor" href="#_4-3-客户端处理获取成员列表响应" aria-label="Permalink to &quot;4.3 客户端处理获取成员列表响应&quot;">​</a></h3><p>套路和前面一样，我们在客户端的 pipeline 中添加一个 handler - <code>ListGroupMembersResponseHandler</code></p><blockquote><p>NettyClient.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>.handler(new ChannelInitializer&lt;SocketChannel&gt;() {</span></span>
<span class="line"><span>    public void initChannel(SocketChannel ch) {</span></span>
<span class="line"><span>        // ...</span></span>
...
<span class="line"><span>        // ...</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>});</span></span></code></pre></div><p>而我们这里 <code>ListGroupMembersResponseHandler</code> 的逻辑也只是在控制台展示一下群成员的信息</p><blockquote><p>ListGroupMembersResponseHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ListGroupMembersResponseHandler extends SimpleChannelInboundHandler&lt;ListGroupMembersResponsePacket&gt; {</span></span>
<span class="line"><span>    </span></span>
