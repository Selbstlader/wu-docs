import{_ as s,c as n,o as e,ag as p}from"./chunks/framework.CPUF7_g-.js";const h=JSON.parse('{"title":"实战：实现客户端与服务端收发消息","description":"","frontmatter":{},"headers":[],"relativePath":"doc/Netty/10.md","filePath":"doc/Netty/10.md"}'),t={name:"doc/Netty/10.md"};function l(c,a,i,o,d,r){return e(),n("div",null,a[0]||(a[0]=[p(`<h2 id="本资源由-itjc8-com-收集整理" tabindex="-1">本资源由 itjc8.com 收集整理 <a class="header-anchor" href="#本资源由-itjc8-com-收集整理" aria-label="Permalink to &quot;本资源由 itjc8.com 收集整理&quot;">​</a></h2><h1 id="实战-实现客户端与服务端收发消息" tabindex="-1">实战：实现客户端与服务端收发消息 <a class="header-anchor" href="#实战-实现客户端与服务端收发消息" aria-label="Permalink to &quot;实战：实现客户端与服务端收发消息&quot;">​</a></h1><blockquote><p>这一小节，我们来实现客户端与服务端收发消息，我们要实现的具体功能是：在控制台输入一条消息之后按回车，校验完客户端的登录状态之后，把消息发送到服务端，服务端收到消息之后打印并且向客户端发送一条消息，客户端收到之后打印。</p></blockquote><h2 id="收发消息对象" tabindex="-1">收发消息对象 <a class="header-anchor" href="#收发消息对象" aria-label="Permalink to &quot;收发消息对象&quot;">​</a></h2><p>首先，我们来定义一下客户端与服务端的收发消息对象，我们把客户端发送至服务端的消息对象定义为 <code>MessageRequestPacket</code>。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Data</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>指令为 <code>MESSAGE_REQUEST ＝ 3</code></p><p>我们把服务端发送至客户端的消息对象定义为 <code>MessageResponsePacket</code></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Data</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>指令为 <code>MESSAGE_RESPONSE = 4</code></p><p>至此，我们的指令已经有如下四种</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface Command {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>    // ...</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这里，我们省去了非关键代码部分</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class LoginUtil {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>如上所示，我们抽取出 <code>LoginUtil</code> 用于设置登录标志位以及判断是否有标志位，如果有标志位，不管标志位的值是什么，都表示已经成功登录过，接下来，我们来实现控制台输入消息并发送至服务端。</p><h2 id="控制台输入消息并发送" tabindex="-1">控制台输入消息并发送 <a class="header-anchor" href="#控制台输入消息并发送" aria-label="Permalink to &quot;控制台输入消息并发送&quot;">​</a></h2><p>在<a href="https://juejin.im/book/5b4bc28bf265da0f60130116/section/5b4daf9ee51d4518f543f130" target="_blank" rel="noreferrer">客户端启动</a>这小节中，我们已经学到了客户端的启动流程，现在，我们在客户端连接上服务端之后启动控制台线程，从控制台获取消息，然后发送至服务端</p><blockquote><p>NettyClient.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span></span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>这里，我们省略了非关键代码，连接成功之后，我们调用 <code>startConsoleThread()</code> 开始启动控制台线程，然后在控制台线程中，判断只要当前 channel 是登录状态，就允许控制台输入消息。</p><p>从控制台获取消息之后，将消息封装成消息对象，然后将消息编码成 <code>ByteBuf</code>，最后通过 <code>writeAndFlush()</code> 将消息写到服务端，这个过程相信大家在学习了上小节的内容之后，应该不会太陌生。接下来，我们来看一下服务端收到消息之后是如何来处理的。</p><h2 id="服务端收发消息处理" tabindex="-1">服务端收发消息处理 <a class="header-anchor" href="#服务端收发消息处理" aria-label="Permalink to &quot;服务端收发消息处理&quot;">​</a></h2><blockquote><p>ServerHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public void channelRead(ChannelHandlerContext ctx, Object msg) {</span></span>
