...
<span class="line"><span>}</span></span></code></pre></div><p>实际生产环境中 <code>Session</code> 中的字段可能较多，比如头像 url，年龄，性别等等。</p><p>然后，我们调用 <code>SessionUtil.bindSession()</code> 保存用户的会话信息，具体实现如下</p><blockquote><p>SessionUtil.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public class SessionUtil {</span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ol><li>服务端在收到客户端发来的消息之后，首先拿到当前用户，也就是消息发送方的会话信息。</li><li>拿到消息发送方的会话信息之后，构造一个发送给客户端的消息对象 <code>MessageResponsePacket</code>，填上发送消息方的用户标识、昵称、消息内容。</li><li>通过消息接收方的标识拿到对应的 channel。</li><li>如果消息接收方当前是登录状态，直接发送，如果不在线，控制台打印出一条警告消息。</li></ol><p>这里，服务端的功能相当于消息转发：收到一个客户端的消息之后，构建一条发送给另一个客户端的消息，接着拿到另一个客户端的 channel，然后通过 <code>writeAndFlush()</code> 写出。接下来，我们再来看一下客户端收到消息之后的逻辑处理。</p><h3 id="_3-3-客户端收消息的逻辑处理" tabindex="-1">3.3 客户端收消息的逻辑处理 <a class="header-anchor" href="#_3-3-客户端收消息的逻辑处理" aria-label="Permalink to &quot;3.3 客户端收消息的逻辑处理&quot;">​</a></h3><blockquote><p>MessageResponseHandler.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public class MessageResponseHandler extends SimpleChannelInboundHandler&lt;MessageResponsePacket&gt; {</span></span>
<span class="line"><span>    @Override</span></span>
