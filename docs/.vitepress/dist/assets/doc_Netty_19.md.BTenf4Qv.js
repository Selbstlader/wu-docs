...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ol><li>首先，通过 groupId 构造群聊响应 <code>GroupMessageResponsePacket</code>，然后再把发送群聊的用户信息填入，这里的用户信息我们就直接复用与 channel 绑定的 session了。</li><li>然后，我们拿到对应群组的 <code>ChannelGroup</code>，通过 <code>writeAndFlush()</code> 写到客户端。</li></ol><p>完整代码大家可以参考 <a href="https://github.com/lightningMan/flash-netty" target="_blank" rel="noreferrer">github</a> <strong>对应本小节分支</strong>，下面进入我们本小节的几个重要知识点，可以拿小本本开始记了。</p><h2 id="_3-共享-handler" tabindex="-1">3. 共享 handler <a class="header-anchor" href="#_3-共享-handler" aria-label="Permalink to &quot;3\\. 共享 handler&quot;">​</a></h2><p>在使用 Netty 完成了一个 IM 系统的核心功能之后，我们再来仔细看一下服务端</p><blockquote><p>NettyServer.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>serverBootstrap</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><ol><li>首先，非常重要的一点，如果一个 handler 要被多个 channel 进行共享，必须要加上 <code>@ChannelHandler.Sharable</code> 显示地告诉 Netty，这个 handler 是支持多个 channel 共享的，否则会报错，读者可以自行尝试一下。</li><li>然后，我们仿照 Netty 源码里面单例模式的写法，构造一个单例模式的类。</li></ol><p>接着，我们在服务端的代理里面就可以这么写</p><blockquote><p>NettyServer.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>serverBootstrap</span></span>
<span class="line"><span>        .childHandler(new ChannelInitializer&lt;NioSocketChannel&gt;() {</span></span>
<span class="line"><span>            protected void initChannel(NioSocketChannel ch) {</span></span>
...
<span class="line"><span>                // ...</span></span>
<span class="line"><span>            }</span></span>
<span class="line"><span>        });</span></span></code></pre></div><p>这样的话，每来一次新的连接，添加 handler 的时候就不需要每次都 new 了，剩下的 8 个 指令，读者可以自行尝试改造一下。</p><h2 id="_4-压缩-handler-合并编解码器" tabindex="-1">4. 压缩 handler - 合并编解码器 <a class="header-anchor" href="#_4-压缩-handler-合并编解码器" aria-label="Permalink to &quot;4\\. 压缩 handler - 合并编解码器&quot;">​</a></h2><p>当我们改造完了之后，我们再来看一下服务端代码</p><blockquote><p>NettyServer.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>serverBootstrap</span></span>
<span class="line"><span>        .childHandler(new ChannelInitializer&lt;NioSocketChannel&gt;() {</span></span>
<span class="line"><span>            protected void initChannel(NioSocketChannel ch) {</span></span>
<span class="line"><span>                ch.pipeline().addLast(new Spliter());</span></span>
<span class="line"><span>                ch.pipeline().addLast(new PacketDecoder());</span></span>
...
<span class="line"><span>   } </span></span>
<span class="line"><span>}</span></span></code></pre></div><p>从上面的抽象逻辑中可以看到，其中只要有一个 channel 的一个 handler 中的 <code>channelRead0()</code> 方法阻塞了 NIO 线程，最终都会拖慢绑定在该 NIO 线程上的其他所有的 channel，当然，这里抽象的逻辑已经做了简化，想了解细节可以参考我关于 Netty 中 NIO 线程（即 reactor 线程）文章的分析， 「<a href="https://www.jianshu.com/p/0d0eece6d467" target="_blank" rel="noreferrer">netty 源码分析之揭开 reactor 线程的面纱（一）</a>」， 「<a href="https://www.jianshu.com/p/467a9b41833e" target="_blank" rel="noreferrer">netty 源码分析之揭开 reactor 线程的面纱（二）</a>」， 「<a href="https://www.jianshu.com/p/58fad8e42379" target="_blank" rel="noreferrer">netty 源码分析之揭开 reactor 线程的面纱（二）</a>」</p><p>而我们需要怎么做？对于耗时的操作，我们需要把这些耗时的操作丢到我们的业务线程池中去处理，下面是解决方案的伪代码</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>ThreadPool threadPool = xxx;</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>        // 4. balabala 其他的逻辑</span></span>
<span class="line"><span>    })</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这样，就可以避免一些耗时的操作影响 Netty 的 NIO 线程，从而影响其他的 channel。</p><h2 id="_7-如何准确统计处理时长" tabindex="-1">7. 如何准确统计处理时长 <a class="header-anchor" href="#_7-如何准确统计处理时长" aria-label="Permalink to &quot;7\\. 如何准确统计处理时长&quot;">​</a></h2><p>我们接着前面的逻辑来讨论，通常，应用程序都有统计某个操作响应时间的需求，比如，基于我们上面的栗子，我们会这么做</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>protected void channelRead0(ChannelHandlerContext ctx, T packet) {</span></span>
<span class="line"><span>    threadPool.submit(new Runnable() {</span></span>
...
<span class="line"><span>    })</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这种做法其实是不推荐的，为什么？因为 <code>writeAndFlush()</code> 这个方法如果在非 NIO 线程（这里，我们其实是在业务线程中调用了该方法）中执行，它是一个异步的操作，调用之后，其实是会立即返回的，剩下的所有的操作，都是 Netty 内部有一个任务队列异步执行的，想了解底层细节的可以阅读一下我的这篇文章 <a href="https://www.jianshu.com/p/feaeaab2ce56" target="_blank" rel="noreferrer">「netty 源码分析之 writeAndFlush 全解析」</a> 因此，这里的 <code>writeAndFlush()</code> 执行完毕之后，并不能代表相关的逻辑，比如事件传播、编码等逻辑执行完毕，只是表示 Netty 接收了这个任务，那么如何才能判断 <code>writeAndFlush()</code> 执行完毕呢？我们可以这么做</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>protected void channelRead0(ChannelHandlerContext ctx, T packet) {</span></span>
<span class="line"><span>    threadPool.submit(new Runnable() {</span></span>
<span class="line"><span>        long begin = System.currentTimeMillis();</span></span>
<span class="line"><span>        // 1. balabala 一些逻辑</span></span>
<span class="line"><span>        // 2. 数据库或者网络等一些耗时的操作</span></span>
