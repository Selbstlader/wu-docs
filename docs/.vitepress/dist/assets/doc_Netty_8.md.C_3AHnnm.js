...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>登录请求数据包继承自 <code>Packet</code>，然后定义了三个字段，分别是用户 ID，用户名，密码，这里最为重要的就是覆盖了父类的 <code>getCommand()</code> 方法，值为常量 <code>LOGIN_REQUEST</code>。</p><p>Java 对象定义完成之后，接下来我们就需要定义一种规则，如何把一个 Java 对象转换成二进制数据，这个规则叫做 Java 对象的序列化。</p><h3 id="序列化" tabindex="-1">序列化 <a class="header-anchor" href="#序列化" aria-label="Permalink to &quot;序列化&quot;">​</a></h3><p>我们如下定义序列化接口</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public interface Serializer {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>    /**</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>序列化接口有三个方法，<code>getSerializerAlgorithm()</code> 获取具体的序列化算法标识，<code>serialize()</code> 将 Java 对象转换成字节数组，<code>deserialize()</code> 将字节数组转换成某种类型的 Java 对象，在本小册中，我们使用最简单的 json 序列化方式，使用阿里巴巴的 <a href="https://github.com/alibaba/fastjson" target="_blank" rel="noreferrer">fastjson</a> 作为序列化框架。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public interface SerializerAlgorithm {</span></span>
<span class="line"><span>    /**</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>然后，我们定义一下序列化算法的类型以及默认序列化算法</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public interface Serializer {</span></span>
<span class="line"><span>    /**</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>编码过程分为三个过程</p><ol><li>首先，我们需要创建一个 ByteBuf，这里我们调用 Netty 的 ByteBuf 分配器来创建，<code>ioBuffer()</code> 方法会返回适配 io 读写相关的内存，它会尽可能创建一个直接内存，直接内存可以理解为不受 jvm 堆管理的内存空间，写到 IO 缓冲区的效果更高。</li><li>接下来，我们将 Java 对象序列化成二进制数据包。</li><li>最后，我们对照本小节开头协议的设计以及上一小节 ByteBuf 的 API，逐个往 ByteBuf 写入字段，即实现了编码过程，到此，编码过程结束。</li></ol><p>一端实现了编码之后，Netty 会将此 ByteBuf 写到另外一端，另外一端拿到的也是一个 ByteBuf 对象，基于这个 ByteBuf 对象，就可以反解出在对端创建的 Java 对象，这个过程我们称作为解码，下面，我们就来分析一下这个过程。</p><h3 id="解码-解析-java-对象的过程" tabindex="-1">解码：解析 Java 对象的过程 <a class="header-anchor" href="#解码-解析-java-对象的过程" aria-label="Permalink to &quot;解码：解析 Java 对象的过程&quot;">​</a></h3><blockquote><p>PacketCodeC.java</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>public Packet decode(ByteBuf byteBuf) {</span></span>
